...
from edisgo.opf.Florian_lopf import setup_model, optimize, prepare_time_invariant_parameters, \
    update_model, create_EV_dict
...
optimize_ev = False
optimize_battery_based_ev_charging = True


def run_optimized_charging_feeder_parallel(grid_feeder_tuple, run='_SEST',
                                           load_results=False, iteration=0, downstream_nodes_matrix=None):
    objective = 'dummy'
    ...
    root_dir = r'C:\Users\Arbeiter\Documents\ETH\MEST\Application_Master_Thesis\Master_Project\Data\testgrid_Florian_fewer_cars'
    ...
    print('Das ist die feeder_dir ' + str(feeder_dir))
    ...
    # try:
    edisgo_orig = import_edisgo_from_files(feeder_dir, import_timeseries=True, import_electromobility=True)

    print('eDisGo object imported.')

    edisgo_obj = convert_impedances_to_mv(edisgo_orig)
    edisgo_obj = create_EV_dict(edisgo_obj, feeder_dir)
    ...
    parameters = prepare_time_invariant_parameters(edisgo_obj, downstream_nodes_matrix, pu=False,
                                                   ...
                                                   battery_optimized_ev_charging=True)
    ...
    for iteration in range(start_iter, int(len(
        ...
        if iteration == 0:
            model = setup_model(parameters, timesteps, objective=objective,
                                ...
                                optimize_battery_based_ev_charging=optimize_battery_based_ev_charging,
            ...
            model = update_model(model, timesteps, parameters, overlap_interations, optimize_storage=optimize_storage,
